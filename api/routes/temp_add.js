      ...
      if ("id" in temp) {
		...
		temperature.sensorId = temp.id;
		  ...
		  temperature.utc_timestamp = temp.utc_timestamp;
	        ...
	        res.status(500);
                res.json({result:"fail", reason:"Property t is not a number"});
              }
            } else {
              console.log("Error json object is missing the t property");
	      res.status(500);
              res.json({result:"fail", reason:"Missing t property"});
            }
          } else {
            console.log("Error id property cannot be empty");
	    res.status(500);
            res.json({result:"fail", reason:"Property id is an empty string"});
          }
        } else {
          console.log("Error id property is not a string");
	  res.status(500);
          res.json({result:"fail", reason:"Property id is not a string"});
        }
      } else {
        console.log("Error json object is missing the id property");
	res.status(500);
        res.json({result:"fail", reason:"Property id is missing"});
      }
    }
  } else {
