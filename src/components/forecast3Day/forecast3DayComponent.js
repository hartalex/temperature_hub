      ...
      .then(function(currentjson) {
        if (currentjson.simpleforecast) {
          if (currentjson.simpleforecast.forecastday) {
            for (var i = 0; i < 3; i++) {
              that.state.data.forecast[i].weatherDescription =
                currentjson.simpleforecast.forecastday[i].conditions
              that.state.data.forecast[
                i
              ].icon = currentjson.simpleforecast.forecastday[
                i
              ].icon_url.replace('http:', 'https:')
              that.state.data.forecast[i].temperatureMax =
                currentjson.simpleforecast.forecastday[i].high.fahrenheit
              that.state.data.forecast[i].temperatureMin =
                currentjson.simpleforecast.forecastday[i].low.fahrenheit
              that.state.data.forecast[i].dt =
                currentjson.simpleforecast.forecastday[i].date.epoch
              that.state.data.forecast[i].pop =
                currentjson.simpleforecast.forecastday[i].pop
            }
          ...
          {this.state.data.forecast.map(obj => (
            <div key={obj.dt} style={this.state.innerStyle}>
              <div style={{ padding: '5px 0', color: Colors.White }}>
                {getWeekDay(new Date(parseInt(obj.dt * 1000, 10)).getDay())}
              </div>
              <img src={obj.icon} alt="forecast" />
              <div style={{ margin: 'auto', width: '100px', fontSize: '25px' }}>
                <div
                  style={{
                    float: 'left',
                    paddingLeft: '20px',
                    color: temperatureColor(obj.temperatureMin)
                  }}
                >
                ...
                <div
                  style={{
                    float: 'left',
                    paddingLeft: '10px',
                    color: temperatureColor(obj.temperatureMax)
                  }}
                >
                  {Math.trunc(obj.temperatureMax)}
                </div>
              </div>
              <div style={{ color: Colors.White, clear: 'left' }}>
