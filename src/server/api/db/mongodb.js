...
module.exports = function (client) {
  ...
  return {
              ...
              return (err, connection) => {
                if (err === null) {
                  ...
                  if (retryCnt < retryMax) {
                    ...
                    setTimeout(() => {
                      client.connect(url, connectCallback(retryMax, retryCnt))
                    }, 15000)
                  } else {
    ...
    updateData: function (db, query, collection, obj) {
      return new Promise(function (resolve, reject) {
        if (db === null) {
          reject('parameter db is null')
        } else if (query === null) {
          reject('parameter query is null')
          ...
          resolve(dbcollection.updateOne(query, obj, { upsert: true, w: 'majority', wtimeout: 100 }))
        }
      })
    },
