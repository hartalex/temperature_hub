  ...
  return {
              ...
              return (err, connection) => {
                if (err === null) {
                  ...
                  if (retryCnt < retryMax) {
                    ...
                    logging.log(
                      "info",
                      "MongoConnectionFailed: No More Retries: " + err
                    );
                    reject(err);
                  }
                }
    ...
    queryData: function(db, query, collection, callback) {
      if (this.checkArgs(db, query, collection, callback)) {
        try {
          ...
          callback(null, dbcollection.find(query).toArray());
        } catch (err) {
          callback(err);
        }
      }
    ...
    querydistinctData: function(db, query, collection, callback) {
      if (this.checkArgs(db, query, collection, callback)) {
        try {
          var dbcollection = db.collection(collection);
          callback(null, dbcollection.distinct(query));
        } catch (err) {
          callback(err);
        }
      }
    ...
    queryOneData: function(db, query, collection, callback) {
      if (this.checkArgs(db, query, collection, callback)) {
        try {
          var dbcollection = db.collection(collection);
          callback(null, dbcollection.findOne(query));
        } catch (err) {
          callback(err);
        }
      }
    ...
    queryAggregateData: function(db, query, collection, callback) {
      if (this.checkArgs(db, query, collection, callback)) {
        try {
          var dbcollection = db.collection(collection);
          callback(null, dbcollection.aggregate(query).toArray());
        } catch (err) {
          callback(err);
        }
      }
