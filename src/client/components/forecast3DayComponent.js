...
class Forecast3DayComponent extends React.Component {
  ...
  getData (that) {
    fetch(ClientConfig.hub_api_url + '/forecast')
        ...
        if (currentjson.simpleforecast) {
          if (currentjson.simpleforecast.forecastday) {
            for (var i = 0; i < 3; i++) {
              ...
              that.state.data.forecast[i].icon = currentjson.simpleforecast.forecastday[i].icon_url.replace(
                'http:',
                'https:'
              )
    ...
    if (this.state.data !== null) {
      ...
      retval = (
        <div style={this.state.style}>
          ...
          {this.state.data.forecast.map((obj) => (
            <div key={obj.dt} style={this.state.innerStyle}>
              ...
              <div style={{ margin: 'auto', width: '100px', fontSize: '15px' }}>
                ...
                <div style={{ float: 'left', paddingLeft: '10px', color: temperatureColor(obj.temperatureMax) }}>
          ...
          <div style={{ clear: 'left', color: Colors.White, fontSize: '7px', textAlign: 'right' }}>
            {updateTimeInMinutes}
          </div>
        </div>
      )
