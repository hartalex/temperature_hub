      ...
      fetchGraphData(duration)
        .then(function(json) {
          ...
          json.data.forEach(function(element) {
            ...
            setDataRow(arrayRow, sensorjson.data, lastArrayRow, element);
            arraydata.push(arrayRow);
            lastArrayRow = arrayRow;
          });
          var arrayRow = JSON.parse(
            JSON.stringify(arraydata[arraydata.length - 1]),
          );
          arrayRow[0] = new Date();
          arraydata.push(arrayRow);
          that.setState({
...
const fetchSensorData = function() {
  return fetch(ClientConfig.hub_api_url + '/temp/sensor/list').then(function(
    response,
  ) {
...
const fetchGraphData = function(duration) {
  return fetch(ClientConfig.hub_api_url + '/temp/' + duration + '/graph').then(
    function(response) {
      if (response.status >= 400) {
        throw new Error('Bad response from server');
      }
...
class TemperatureGraph extends LineGraphComponent {
  constructor(props) {
    super(props, 'TemperatureGraph', getdata);
  }
}
